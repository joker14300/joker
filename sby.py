...
def executeCmd(msg, text, txt, cmd, msg_id, receiver, sender, to, setKey):
    if cmd == '@logoutbot':
        jokerLove(to, 'Bot will logged out')
        sys.exit('##----- PROGRAM STOPPED -----##')
#    if cmd == "creator":
#        joker(to,"Contact Dibawah Ini \nAdalah Creator Bot")
#        line.sendContact(to,"u7f326a34d7429023bd648aab804148d9")
    if cmd == "listoken":
    ...
    if cmd == "help":
      ...
      "body": {
        ...
        "contents": [
          {
            "type": "image",
            "url": str(line.getProfileCoverURL("u65129423a63f75ecae3ea189f9540054")),
            "size": "full",
            "aspectMode": "cover",
            "aspectRatio": "2:3",
            "gravity": "top"
    ...
    if cmd == "gif":
        ...
        line.sendReplyMessage(msg_id, to, 'line://app/1602687308-GXq4Vvk9?type=profile')
#    elif cmd == 'me':
#           msg.contentType = 13
 ...
 #     if msg._from in Owner:
        ...
        if texttl == 'on':
            if settings['checkContact']:
                jokerLove(to, 'Checkcontact already active')
            else:
                settings['checkContact'] = True
                yepzLove(to, 'Success activated checkcontact')
        elif texttl == 'off':
            if not settings['checkContact']:
                jokerLove(to, 'Checkcontact already deactive')
            else:
        ...
        if texttl == 'on':
            if settings['checkPost']:
                jokerLove(to, 'Checkpost already active')
            else:
                settings['checkPost'] = True
                jokerLove(to, 'Success activated checkpost')
        elif texttl == 'off':
            if not settings['checkPost']:
                jokerLove(to, 'Checkpost already deactive')
            else:
                settings['checkPost'] = False
                jokerLove(to, 'Success deactivated checkpost')
    elif cmd.startswith('checksticker '):
        textt = removeCmd(text, setKey)
        texttl = textt.lower()
        if texttl == 'on':
            if settings['checkSticker']:
                jokerLove(to, 'Checksticker already active')
            else:
                settings['checkSticker'] = True
                jokerLove(to, 'Success activated checksticker')
        elif texttl == 'off':
            if not settings['checkSticker']:
                jokerLove(to, 'Checksticker already deactive')
            else:
                settings['checkSticker'] = False
                jokerLove(to, 'Success deactivated checksticker')
        ...
        if cmd == 'myprofile':
            ...
            jokerLove(to, '「 Display Name 」\n' + str(profile.displayName))
        elif texttl == 'bio':
            jokerLove(to, '「 Status Message 」\n' + str(profile.statusMessage))
        elif texttl == 'pict':
            if profile.pictureStatus:
                path = 'http://dl.profile.line-cdn.net/' + profile.pictureStatus
                line.sendImageWithURL(to, path)
                jokerLove(to, '「 Picture Status 」\n' + path)
            else:
                jokerLove(to, 'Failed display picture status, user doesn\'t have a picture status')
        elif texttl == 'cover':
            cover = line.getProfileCoverURL(profile.mid)
            line.sendImageWithURL(to, str(cover))
            jokerLove(to, '「 Cover Picture 」\n' + str(cover))
            ...
            if textsl.startswith('name '):
                ...
                if len(name) <= 20:
                    ...
                    line.updateProfile(profile)
                    jokerLove(to, 'Success change display name, changed to `%s`' % name)
                else:
                    jokerLove(to, 'Failed change display name, the length of the name cannot be more than 20')
                ...
                if len(bio) <= 500:
                    ...
                    jokerLove(to, 'Failed change status message, the length of the bio cannot be more than 500')
            elif textsl == 'pict':
                settings['changePictureProfile'] = True
                jokerLove(to, 'Please send the image to set in picture profile, type `{key}Abort` if want cancel it.\nFYI: Downloading images will fail if too long upload the image'.format(key=setKey.title()))
            elif textsl == 'cover':
                settings['changeCoverProfile'] = True
                jokerLove(to, 'Please send the image to set in cover profile, type `{key}Abort` if want cancel it.\nFYI: Downloading images will fail if too long upload the image'.format(key=setKey.title()))
            else:
                yepzLove(to, parsingRes(res).format_map(SafeDict(key=setKey.title())))
        else:
            jokerLove(to, parsingRes(res).format_map(SafeDict(key=setKey.title())))
        ...
        if cmd == 'profile':
            ...
            if profile.pictureStatus:
                ...
                jokerzLove(to, 'Failed display picture status, user doesn\'t have a picture status')
        elif texttl == 'cover':
            if msg.toType != 0: return yepzLove(to, 'Failed display mid user, use this command only in personal chat')
            cover = line.getProfileCoverURL(profile.mid)
            line.sendImageWithURL(to, str(cover))
            ...
            if textsl.startswith('profile '):
                if 'MENTION' in msg.contentMetadata.keys():
                    ...
                    jokerLove(to, 'Failed steal profile, no one user mentioned')
                ...
                if 'MENTION' in msg.contentMetadata.keys():
                    ...
                    jokerLove(to, 'Failed steal mid, no one user mentioned')
                ...
                if 'MENTION' in msg.contentMetadata.keys():
                    ...
                    jokerLove(to, 'Failed steal display name, no one user mentioned')
            elif textsl.startswith('bio '):
                res = '╭───「 Status Message 」'
                no = 0
                if 'MENTION' in msg.contentMetadata.keys():
                    ...
                    jokerLove(to, 'Failed steal status message, no one user mentioned')
                ...
                if 'MENTION' in msg.contentMetadata.keys():
                    ...
                    if len(mentions['MENTIONEES']) == 1:
                        ...
                        if profile.pictureStatus:
                            ...
                            return jokerLove(to, 'Failed steal picture status, user `%s` doesn\'t have a picture status' % profile.displayName)
                    for mention in mentions['MENTIONEES']:
                        mid = mention['M']
                        profile = line.getContact(mid)
                        no += 1
